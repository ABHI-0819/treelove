        ...
        listener: (context, state) {
          if (state is ApiLoading) {
            EasyLoading.show();
              ...
              is ApiSuccess<PlantationResponseModel, ResponseModel>) {
            EasyLoading.dismiss();
            _resetForm();
            showNotification(
                type: Not.success, context, message: state.data.message);
  ...
  Future<void> _onSubmit() async {
    ...
    if (lat == null || lng == null) {
      showNotification(context,
          type: Not.warning,
          message: "Please enter valid latitude & longitude");
    }
    if (treeImages.isEmpty) {
      showNotification(context,
          type: Not.warning,
          message: "At least one picture needs to be upload");
    }
    ...
    return Expanded(
      child: Container(
        margin: const EdgeInsets.symmetric(
            horizontal: 4), // slight spacing between buttons
        child: OutlinedButton(
          style: OutlinedButton.styleFrom(
            backgroundColor:
                isSelected ? const Color(0xFFE6F0ED) : Colors.white,
            side: BorderSide(
              color:
                  isSelected ? const Color(0xFF0E5D57) : Colors.grey.shade400,
              width: 1.3,
            ),
            shape: RoundedRectangleBorder(
              borderRadius:
                  BorderRadius.circular(8), // less rounded = cleaner look
            ),
            padding:
                const EdgeInsets.symmetric(vertical: 14), // balanced height
            elevation: 0,
          ),
